      ...
      <div class="grid grid-cols-1 xl:grid-cols-2 gap-12">
        ...
        <div class="space-y-6">
          ...
          <div class="relative group">
            <div class="aspect-square bg-gradient-to-br from-slate-800 to-slate-700 rounded-3xl overflow-hidden border border-slate-600">
              <img 
            ...
            <!-- Image Navigation -->
            <div v-if="product.product_images?.length > 1" class="absolute top-1/2 -translate-y-1/2 left-4 right-4 flex justify-between pointer-events-none">
              <button class="w-12 h-12 bg-slate-900/80 backdrop-blur-md rounded-full border border-slate-600 flex items-center justify-center text-white hover:bg-slate-800 transition-colors pointer-events-auto">
          ...
          <!-- Thumbnail Gallery -->
          <div v-if="product.product_images?.length" class="grid grid-cols-4 gap-3">
            <button 
              v-for="image in product.product_images" 
              :key="image.id"
              @click="selectedImage = image.image_url"
              class="aspect-square bg-gradient-to-br from-slate-800 to-slate-700 rounded-xl overflow-hidden border-2 transition-all duration-300"
        ...
        <div class="space-y-8">
          ...
          <div>
            ...
            <h1 class="text-4xl font-black text-transparent bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text mb-4">
              {{ product.name }}
            </h1>
          ...
          <div class="bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-700/50 p-6">
            <div class="flex items-baseline gap-4">
              <span class="text-4xl font-black text-transparent bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text">
                ${{ currentPrice }}
              </span>
          ...
          <!-- Variants -->
          <div v-if="activeVariants.length" class="bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-700/50 p-6">
            <h3 class="text-xl font-bold text-white mb-4">Choose Options</h3>
            <div class="space-y-3">
              <label v-for="variant in activeVariants" :key="variant.id" 
                     class="flex items-center justify-between p-4 rounded-xl border-2 cursor-pointer transition-all duration-300"
                ...
                <div class="text-right">
                  <div v-if="variant.discount_price" class="space-y-1">
                    <div class="text-xl font-bold text-cyan-400">${{ parseFloat(variant.discount_price).toFixed(2) }}</div>
                    <div class="text-sm text-slate-500 line-through">${{ parseFloat(variant.regular_price).toFixed(2) }}</div>
                  </div>
                  <div v-else class="text-xl font-bold text-white">${{ parseFloat(variant.regular_price).toFixed(2) }}</div>
                </div>
              </label>
            </div>
          </div>
          ...
          <!-- Quantity & Actions -->
          <div class="bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-700/50 p-6 space-y-6">
            <!-- Quantity -->
            <div>
              <label class="block text-sm font-medium text-slate-300 mb-3">Quantity</label>
              <div class="flex items-center gap-4">
                <div class="flex items-center bg-slate-900 rounded-xl border border-slate-600">
                  <button 
                    @click="quantity = Math.max(1, quantity - 1)"
                    class="p-3 text-slate-400 hover:text-cyan-400 transition-colors"
                  >
                  ...
                  <input 
                    ...
                    min="1" 
                    :max="totalStock"
                    class="w-20 px-3 py-3 bg-transparent text-center text-white font-semibold focus:outline-none"
                  >
                  <button 
        ...
        <div class="bg-slate-800/50 backdrop-blur-xl rounded-3xl border border-slate-700/50 overflow-hidden">
          ...
          <div class="flex border-b border-slate-700">
            <button 
              ...
              class="px-8 py-4 font-semibold transition-all duration-300"
          ...
          <div class="p-8">
            ...
            <div v-if="activeTab === 'description'" class="prose prose-invert max-w-none">
              <div class="text-slate-300 leading-relaxed" v-html="product.description || 'No description available.'">
              </div>
            </div>
            
            <!-- Specifications Tab -->
            <div v-if="activeTab === 'specifications'" class="space-y-4">
              <div v-if="product.product_attributes?.length" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div v-for="attr in product.product_attributes" :key="attr.id" 
                     class="flex justify-between p-4 bg-slate-900/50 rounded-xl border border-slate-700">
                  <span class="font-medium text-slate-300">{{ attr.key }}</span>
                  <span class="text-white font-semibold">{{ attr.value }}</span>
                </div>
              </div>
            ...
            <div v-if="activeTab === 'reviews'" class="space-y-8">
              <!-- Add Review Form -->
              <div v-if="user" class="bg-slate-900/50 rounded-2xl border border-slate-700 p-6">
                ...
                <form @submit.prevent="submitReview" class="space-y-6">
                  ...
                  <div>
                    ...
                    <textarea 
                      ...
                      rows="4"
                      class="w-full px-4 py-3 bg-slate-900 border border-slate-600 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:border-cyan-500 transition-colors"
                      placeholder="Share your experience with this product..."
                    ></textarea>
                  </div>
                  <UButton type="submit" class="bg-gradient-to-r from-cyan-500 to-purple-500 hover:from-cyan-400 hover:to-purple-400">
                    Submit Review
                  </UButton>
                </form>
              </div>
              ...
              <div class="space-y-6">
                <div v-for="review in product.reviews" :key="review.id" 
                     class="bg-slate-900/50 rounded-2xl border border-slate-700 p-6">
                  <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center gap-4">
                      ...
                      <span class="font-medium text-white">{{ review.users?.email || 'Anonymous' }}</span>
                    </div>
                ...
                <div v-if="!product.reviews?.length" class="text-center py-12 text-slate-400">
